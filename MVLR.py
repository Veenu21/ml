...
mydata = pd.read_csv("Data_MVLR.csv")
print(mydata)
print(type(mydata))
print(mydata.info())

#Separate the features from target
X_input = mydata.iloc[:,0:4]
Y_out = mydata.iloc[:,4]
print(Y_out.shape)
...
X_input_nu = mydata[['Petrol_tax', 'Average_income' , 'Paved_Highways' , 'Population_Driver_licence(%)','Petrol_Consumption']]
...
plt.scatter(X_input_nu.Petrol_tax,Y_out.values,color='blue')
plt.show()

plt.scatter(X_input_nu.Average_income,Y_out.values,color='red')
plt.show()

plt.scatter(X_input_nu.Paved_Highways,Y_out.values,color='red')
plt.show()

plt.scatter(X_input_nu.iloc[:,3],Y_out.values,color='red')
plt.show()

#Predicting PETROLTAX, PETROLCONSUMPTION
XX=X_input_nu.iloc[:,3]
YY=Y_out
